👋 Olá, me chamo Leonardo!
🚀 Bem-vindo ao meu perfil no GitHub!
Sou um entusiasta do desenvolvimento web e estudante de Análise e Desenvolvimento de Sistemas na Uninter. Sempre em busca de novos desafios e aprendizados, atualmente estudo JavaScript, TypeScript, React e desenvolvimento Front-End.

🌱 Atualmente estou aprendendo:
 JavaScript, HTML5 e CSS3 para desenvolvimento web moderno;
   React e React Native para criação de aplicações interativas e responsivas;
TypeScript para uma tipagem mais segura e escalável no desenvolvimento;
Programação Orientada a Objetos (POO) com JavaScript e TypeScript.
💡 Estou buscando colaborar em:
Projetos de Front-End utilizando Javascript, HTML5, CSS3, React e Vite entre outras;
Integração com APIs para consumo e exibição de dados dinâmicos;
Desenvolvimento de sites e aplicativos interativos.
🤝 Estou aberto a receber ajuda em:
Boas práticas e padrões de código;
Testes automatizados e TDD;
Melhoria na performance e otimização de aplicações.
📍 Onde estou:
Moro no Paraná, Brasil.
📫 Como me encontrar:
LinkedIn:<a href="https://www.linkedin.com/in/leonardo-feliciano-da-costa-3a760b200/" target="_blank">
  <i class="devicon-linkedin-plain colored"></i>
</a>
Email: leonardofeliciano81@gmail.com
🎯 Curiosidades sobre mim:
Além de programar, estou investindo no desenvolvimento pessoal com PNL (Programação Neurolinguística);
Já desenvolvi um site de previsão do tempo e estou criando um app para isso;
Trabalho em projetos freelancers, como sites para empresas e sistemas personalizados.
